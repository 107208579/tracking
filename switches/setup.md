...
→ Set your keyboard layout to US
...
→ Follow the instructions for flashing the CC2652 chip
https://www.zigbee2mqtt.io/information/supported_adapters.html
 https://electrolama.com/projects/zig-a-zig-ah/#flash-firmware

→ The following steps are outlined for macOS
→ Connect the CC2652 via USB and check if it was detected
`sudo dmesg | grep AppleUSBCH`
...
→ Upgrade 'pip' and install the following packages
...
→ Check the exact device location the CC2652 is connected to
`ls -l /dev/tty.usbserial-*`
⁃ /dev/tty.usbserial-1460


→ Flash the CC2652 by pressing the reset button on the board
→ Keep pressing while plugging the device back into the USB port
→ Release the button after 3 seconds
→ Run the following command (note the device location)
`./cc2538-bsl.py -p /dev/tty.usbserial-1460 -evw CC26X2R1_20201026.hex`
⁃ Opening port /dev/tty.usbserial-1460, baud 500000
⁃ Reading data from CC26X2R1_20201026.hex
⁃ Your firmware looks like an Intel Hex file
⁃ Connecting to target...
⁃ CC1350 PG2.0 (7x7mm): 352KB Flash, 20KB SRAM, CCFG.BL_CONFIG at 0x00057FD8
⁃ Primary IEEE Address: 00:12:4B:00:21:B7:77:5E
...
→ Connect your Zigbee receiver to the Raspberry Pi USB port
...
→ For troubleshooting run any of tthe following commands
`sudo lsusb`
...
→ Install dependencies (this may take ~20 minutes on a Rasperry Pi Zero)
`npm ci --production`
...
→ Quickly check if your node started up and runs correctly
`cd /opt/zigbee2mqtt`
`npm start `
...
→ Example: Ikea Tradfri E1524/E1810 (Round Remote Control)
...
→ Example: Sonoff SNZB-01 (Square Button)
...
→ Example:XiaoMi Aqara WXKG01LM (Round Button)
...
→ Check your configuration file again and see if all devices have been added successfully
        less /opt/zigbee2mqtt/data/configuration.yaml
...
1sudo systemctl start zigbee2mqtt1
...
→ The Mosquitto client allows to format the data output - see some examples below
...
→ Download 'mqtt2caldav' to your servers home directory
...
→ Or automatically start the script after upon system boot
`crontab -e`
